...
- Some extra debug logging is currently enabled in the Consul code and should be quieted/removed
  before any production use (verbose logging in `internal/physical/consul/consul.go` and
  `internal/serviceregistration/consul/consul.go`).
...
2. Expect conflicts mainly in `go.mod` / `go.sum` (large upstream refactors that move
   directories can also require relocating the fork files to match). `go.sum` is set to `merge=union` in
...
3. `go mod tidy` — reconciles module requirements. The Consul dependencies are retained because
   `internal/command/commands_consul.go` imports them.
4. `go build ./... && go vet ./...`
...
OPENBAO_CONSUL_TEST=1 CONSUL_HTTP_ADDR=127.0.0.1:8500 \
  ...
  ./internal/physical/consul/... ./internal/serviceregistration/consul/...
```
